...
static void Cmd_attackcanceler(void)
{
    ...
    && !((gCurrentMove == MOVE_SURF || gCurrentMove == MOVE_BOOMBURST) && (gHitMarker & HITMARKER_NO_PPDEDUCT))
...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        if (gBattleMons[gBattlerAttacker].ability == ABILITY_HUSTLE && IS_TYPE_PHYSICAL(type) && gBattleMoves[gCurrentMove].power >0)
...
static void Cmd_adjustnormaldamage(void)
{
	...
	if (!(gBattleMons[gBattlerTarget].status2 & STATUS2_SUBSTITUTE) && gBattleMons[gBattlerTarget].ability == ABILITY_STURDY && gBattleMons[gBattlerTarget].hp <= gBattleMoveDamage && gBattleMons[gBattlerTarget].hp == gBattleMons[gBattlerTarget].maxHP)
    {
...
static void Cmd_resultmessage(void)
{
    ...
    else
    {
        ...
        case MOVE_RESULT_FOE_ENDURED:
            ...
            stringId = STRINGID_PKMNPROTECTEDBY;
            break;
        case MOVE_RESULT_FAILED:
        ...
        default:
            ...
            else if (gMoveResultFlags & MOVE_RESULT_ONE_HIT_KO)
            {
	            ...
	            else if ((gMoveResultFlags & MOVE_RESULT_FOE_STURDY) == MOVE_RESULT_FOE_STURDY)
            {
                gMoveResultFlags &= ~(MOVE_RESULT_FOE_ENDURED | MOVE_RESULT_FOE_HUNG_ON | MOVE_RESULT_FOE_STURDY);
                BattleScriptPushCursor();
            ...
            case MOVE_EFFECT_WRAP:
                ...
                else
                {
                    ...
                    *(gBattleStruct->wrappedBy + gEffectBattler) = gBattlerAttacker;
                    ...
                    {
                        // Increase this number when increasing gTrappingMoves
...
static void Cmd_getexp(void)
{
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            {
                if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES) == SPECIES_NONE || GetMonData(&gPlayerParty[i], MON_DATA_HP) == 0 || (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES2) == SPECIES_EGG && (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)))
                    continue;
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (!(gBattleTypeFlags & BATTLE_TYPE_TRAINER) && gBattleMons[0].hp && !gBattleStruct->wildVictorySong)
                {
				 ...
				 if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP) && !(GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPECIES2) == SPECIES_EGG && (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)))
                {
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
			...
			if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP) && !(GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_SPECIES2) == SPECIES_EGG && (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)) && GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) != MAX_LEVEL)
            {
    ...
    else
    {
		...
		// NUEVO PARA CUSTOM TRAINER
		 case F_TRAINER_PARTY_CUSTOM_MIDELE:
            {
                const struct TrainerMonCustomMidele *party = gTrainers[trainerId].party.ItemCustomMidele;
                lastMonLevel = party[gTrainers[trainerId].partySize - 1].lvl;
            }
            break;
		// NUEVO PARA CUSTOM TRAINER
...
static void Cmd_displaydexinfo(void)
{
    ...
    case 1:
        ...
        {
            FreeAllWindowBuffers();
            gBattleCommunication[TASK_ID] = CreateDexDisplayMonDataTask(species,
