...
const u8 gTypeEffectiveness[369] = // NUEVO MODIFICADO TIPO HADA, ANTES 336
{
...
static void sub_80379F8(u8 arrayIdPlus)
{
    ...
    for (i = 0; i < (int)ARRAY_COUNT(gMultiPartnerParty); i++)
    {
        ...
        if (GetLanguage(&gPlayerParty[arrayIdPlus + i]) != LANGUAGE_JAPANESE)
...
static void HandleAction_ThrowPokeblock(void)
{
    ...
    {
	// BUG FIX: safariEscapeFactor can become 0 below. This causes the pokeblock throw glitch.
